...
pub struct VestingInfo {
    pub vestingUnlockTimestamp: U256,           // 32
    pub vestingUnlockPercentage: U256,          // 32
}
...
pub struct Sale {
    ...
    // vesting info
    pub isVested: bool,
    pub vestingLen: usize,
    pub vestingInfo: Vec<VestingInfo>, // 4 + VestingInfo::LEN * vestingLen
}
...
impl VestingInfo {
    ...
    pub fn make_from_slice(bytes: &[u8]) -> Result<Self> {
        require!(
            bytes.len() == VestingInfo::LEN,
            ContributorError::InvalidVestingInfoPayload
        );
        Ok(Self {
            vestingUnlockTimestamp: U256::from_big_endian(&bytes[0..32]),
            vestingUnlockPercentage: U256::from_big_endian(&bytes[32..64])
        })
    }
}
...
impl Sale {
    pub const MAXIMUM_SIZE: usize = 32
        ...
        + 1
        + 1
        + (4 + VestingInfo::LEN * VESTING_LENGTH_MAX);
    ...
    pub fn parse_sale_init(&mut self, payload: &[u8]) -> Result<()> {
        ...
        // set the vesting info
        self.isVested = bool::try_from(payload[recipient_idx + 52 + 24 + 8])?;
        self.vestingLen = payload[recipient_idx + 52 + 24 + 8 + 1] as usize;
        require!(
            self.vestingLen <= VESTING_LENGTH_MAX,
            ContributorError::TooManyVestingPlan
        );
        for i in 0..self.vestingLen {
            let start = recipient_idx + 52 + 24 + 8 + 1 + 1 + i * VestingInfo::LEN;
            self.vestingInfo.push(
                VestingInfo::make_from_slice(&payload[start..start+VestingInfo::LEN])
            );
        }
        Ok(())
    }
